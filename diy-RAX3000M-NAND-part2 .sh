...
# Modify hostname
sed -i 's/immortalwrt/LSYDY-Router/g' package/base-files/files/bin/config_generate

# Modify filename, add date prefix
#sed -i 's/IMG_PREFIX:=/IMG_PREFIX:=AP-$(shell date +"%Y%m%d")-/1' include/image.mk
#sed -i 's/IMG_PREFIX:=/IMG_PREFIX:=$(shell date +"%Y%m%d")-24.10/1' include/image.mk
sed -i 's/IMG_PREFIX:=/IMG_PREFIX:=$(shell date +"%Y%m%d")-/1' include/image.mk

#rm -rf feeds/packages/utils/watchcat
#git clone -b openwrt-24.10 --single-branch --filter=blob:none https://github.com/openwrt/packages feeds/packages/utils/watchcat
